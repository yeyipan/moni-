...
app.factory('apiFoods', ['$http', function ($http) {
    ...
    // 获取所有的数组分类/
    s.getBooksTypes = function (cb) {
        $http.get('http://localhost:3000/api/v1/book/all_type')
            .then(function (res) {
                // 回调函数里面是服务端响应的数据//
    ...
    // 获取商品数据／／
    s.getBooks = function (type, cb) {
        $http.get('http://localhost:3000/api/v1/book/?type=' + type).then(function (res) {
            cb(res.data)
        })
    }
